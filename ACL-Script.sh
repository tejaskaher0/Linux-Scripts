...
# Advanced User and Group Management System Script
# With Custom ACL Permission Input + Colored Output
...
create_user() {
    ...
    if id "$username" &>/dev/null; then
        ...
        success "Password policy set for user $username (Max: 90 days, Min: 7 days, Warn: 7 days)."
        log_action "Password policy applied for $username."
    fi
}

# -------------------------------
# 🔐 Set ACL Permissions (Custom)
# -------------------------------
set_acl() {
    ...
    success "ACL permission '$permissions' set for user '$acl_user' on directory '$directory'."
...
backup_users() {
    ...
    success "Backup of /home created at $BACKUP_FILE."
...
while true; do
    echo ""
    echo "------------------------------------"
    echo -e "${YELLOW}  Advanced User & Group Management  ${RESET}"
    echo "------------------------------------"
    echo "1. Create User and Group"
    echo "2. Set ACL Permissions (Custom)"
    echo "3. Backup User Homes"
    echo "4. Monitor Active Users"
    echo "5. Exit"
    echo "------------------------------------"
    ...
    case "$choice" in
        ...
        3) backup_users ;;
        4) monitor_users ;;
        5) info "Exiting script. Thank you!"
           exit 0 ;;
